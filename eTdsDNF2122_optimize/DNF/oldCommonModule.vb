...
Imports eTdsDNF1920.IVRStatusSvc
...
Public Module CommonModule
    ...
    Public Const dcmDelayeDepositIntrst As Decimal = 0.18
    ...
    'Ver 8.002-FastFact Start
    Dim cp As New CheckPAN.VerifyPAN
    'Ver 8.002-FastFact End
    'Ver 7.03-REQ816 end 
    ...
    Public Sub DeducteeDetails()
        ...
        dtDD.Columns.Add("Dcode", System.Type.GetType("System.Char"))
        ...
        dtDD.Columns.Add("PrescribedRate", System.Type.GetType("System.Single"))
            ...
            Do Until strRd.EndOfStream = True
                ...
                ElseIf mCode = "BH" Then
                    ...
                    ElseIf values(16) < "201011" Or values(16) > "201920" Then 'Fin Year
                        ...
                        'Ver 7.03-REQ797 end
                        'Ver 6.00-REQ689 end
                        MessageBox.Show("Please select file of financial year 2010-2011, 2011-2012, 2012-2013 or 2013-2014 or 2014-2015 or 2015-2016 or 2016-2017 or 2017-2018 or 2018-2019 only.", "DNF", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        'Ver 5.2-REQ590 end 
                        'Ver 3.01-REQ303 end
                        'Ver 1.01-E590 End
                        blnFileClose = True
...
contn:
    ...
    Public Sub UpdateRates()
        Try
            ...
            For i = 0 To intRateMasterCount - 1
                ...
                If strFormNo = "26Q" Or strFormNo = "27EQ" Or strFormNo = "27Q" Then
                    dtTempView.RowFilter = "DeducteeStatus = '" & dtRateMaster.Rows(i)("DeducteeStatus") & _
                                           "' And Section = '" & dtRateMaster.Rows(i)("Section") & "'"

                End If
                For j = 0 To dtTempView.Count - 1
                    'Ver 2.02-REQ234 start
                    ' If strFormNo = "26Q" Or strFormNo = "27EQ" Then
                    If strFormNo = "26Q" Or strFormNo = "27EQ" Or strFormNo = "27Q" Then
                        'Ver 2.02-REQ234 end
                        'For Valid PANs and with No Reason for Lower/Non deduction

                        If (dtTempView.Item(j)("PANStatus") = "V" Or dtTempView.Item(j)("PANStatus") = "X") And dtTempView.Item(j)("Remark1") = "" Then
                            '94I has 2 types of rate irrespective of deductee status, Hence below logic is applied
                            If dtRateMaster.Rows(i)("Section") = "94I" Then
                                strRate = dtRateMaster.Rows(i)("Rate").ToString.Split(",")
                                'If dtRateMaster.Rows(i)("Rate") > 2 Then
                            ...
                            ElseIf dtRateMaster.Rows(i)("Section") = "94C" Then
                                'Dcode
                                strRate = dtRateMaster.Rows(i)("Rate").ToString.Split(",")
                                ...
                                End If
                                Array.Resize(strRate, 0)
                                'Ver 5.2-REQ590 start --94H Rate changes 
                            ElseIf dtRateMaster.Rows(i)("Section") = "94H" Or dtRateMaster.Rows(i)("Section") = "94G" Or dtRateMaster.Rows(i)("Section") = "4EE" Or dtRateMaster.Rows(i)("Section") = "4DA" Then
                                ...
                                'dtDateDeducted94H = Mid(dtTempView.Item(j)("DateDeducted"), 1, 2) & "/" & Mid(dtTempView.Item(j)("DateDeducted"), 3, 2) & "/" & Mid(dtTempView.Item(j)("DateDeducted"), 5, 4) ' Date of Deducted
                            ...
                            'For Invalid PANs Update PENAL Rate of IT Rate Chart   
                        ElseIf dtTempView.Item(j)("PANStatus") = "I" Or dtTempView.Item(j)("PANStatus") = "N" Then
                            dtTempView.Item(j)("PrescribedRate") = dtRateMaster.Rows(i)("Rate")
                            'For Valid PANs and with  Reason for Lower/Non deduction OR PAN not Verified with ITD Database,
                            'Do not consider IT Rate Chart
                            ' ElseIf (dtTempView.Item(j)("PANStatus") = "V" And dtTempView.Item(j)("Remark1") <> "") Or dtTempView.Item(j)("PANStatus") = "X" Then
                    ...
                    If strFormNo = "26Q" Or strFormNo = "27EQ" Or strFormNo = "27Q" Then
                        ...
                        'Ver 2.02-REQ234 start
                        'If strFormNo = "27Q" Then
                        '    If dtTempView.Item(j)("PANStatus") = "I" Or dtTempView.Item(j)("PANStatus") = "N" Then
                        '        dtTempView.Item(j)("PrescribedRate") = dtRateMaster.Rows(i)("Rate")
                        '    Else
                        '        dtTempView.Item(j)("PrescribedRate") = dtTempView.Item(j)("Rate") ' Tds Rate From Text File
                        '    End If
                    ...
                    'Ver 2.01-REQ233 Start
                    'dtTempView.Item(j)("TotInterest") = dtTempView.Item(j)("DelayedDedAmt") + dtTempView.Item(j)("DelayedDepAmt")
                    dtTempView.Item(j)("TotInterest") = Val(dtTempView.Item(j)("DelayedDedAmt")) + Val(dtTempView.Item(j)("DelayedDepAmt"))
                    'Ver 2.01-REQ233 End
                    'Update Reasons........ Remarks
                    'Remarks
            ...
            If strFinYear >= "201819" Then
                If strFormNo = "27Q" Then
                    ...
                    For i = 0 To intRateMasterCount - 1
                        ...
                        dtTempView.RowFilter = "DTTA = '" & IIf(dtNRIRateMaster.Rows(i)("Flag") = "DTAA", "B", "A") & _
    ...
    Public Sub Tmp(ByVal sender As Object, ByVal e As System.Windows.Forms.WebBrowserDocumentCompletedEventArgs) Handles pIE1.DocumentCompleted
        ...
        Dim pIEdocHtml As MSHTML.HTMLDocument
        'Ver 4.042-QC?? end
        MessageBox.Show("Ok")
        pIE1.Document.InvokeScript("$('#pmgky').dialog('close');")
        pIE1.Document.InvokeScript("$('.sec_btn').click();")
        pIEdocHtml = pIE1.Document.DomDocument
        DocComplete = True
    End Sub
...
takeAction:
    ...
    Public Sub PanVerification()
        ...
        WebPageLoad() 'For Testing

        'Dim sPANQuery As String
        'Deductee Details PAN Checking here
        Try
           
            'Ver 8.002 - Id '0129703' 12/07/19  Start
            Dim sPanQuery As String = ""
            If Not intTempPANCount > 0 Then
                GoTo SD
            End If
            For i = 0 To intTempPANCount - 1
                If (i = intTempPANCount - 1) Then
                    sPanQuery = sPanQuery & dtTempPAN.Rows(i)(0).ToString()
                Else
                    sPanQuery = sPanQuery & dtTempPAN.Rows(i)(0).ToString() & ", "
                End If
            Next

            'Call API server for PAN Verification
            Dim sAPIPath As String
            Dim sPANnumbers As String
            Dim fStream As FileStream
            Dim strRd As StreamReader
            Dim webResponse As JObject


            If File.Exists(Application.StartupPath & "\eTDSDNF.sys") Then
                fStream = New FileStream(Application.StartupPath & "\eTDSDNF.sys", FileMode.Open, FileAccess.Read)
                strRd = New StreamReader(fStream)
                Do Until strRd.EndOfStream = True
                    strLine = strRd.ReadLine()
                    If strLine.Split("=")(0) = "CheckPanAPI" Then
                        sAPIPath = strLine.Split("=")(1)
                    End If
                Loop
            Else
                sAPIPath = ""
            ...
            'Try
            '    webResponse = PostDatatoAPI(sAPIPath, sPanQuery)
            'Catch ex As Exception
            '    MessageBox.Show("Unable to connect API, Please try after 30 minutes")
            'End Try
            Dim Res As JObject = JObject.Parse("{'ADCPT4829P': '1', 'BHWPR4467C' : '0', 'BHWPH4467C' : '3'  }")
            Dim results As List(Of JToken) = Res.Children().ToList
            For Each item As JProperty In results
                ...
                Catch ex2 As Exception
                    ...
                    For l = 0 To dtTempView.Count - 1
                        dtTempView.Item(l)("PANStatus") = "X"
                    Next
                    For index As Integer = 0 To dtTempPAN.Rows.Count - 1
                        If dtTempPAN.Rows(index)("PAN").ToString() = item.Name Then
                            dtTempPAN.Rows(index).Delete()
                            Exit For
                        End If
                    Next
                    'dtTempPAN.Rows(0).Delete()
...
takeAction:
                ...
                If isValid = True Then
                    intCountOfValidPAN = intCountOfValidPAN + 1
                    'strValidPANList.Append("," & dtTempPAN.Rows(0)("PAN").ToString)
                    strValidPANList.Append("," & item.Name)
                    For index As Integer = 0 To dtTempPAN.Rows.Count - 1
                        If dtTempPAN.Rows(index)("PAN").ToString() = item.Name Then
                            dtTempPAN.Rows(index).Delete()
                            Exit For
                ...
                Else
                    intCntInvalidPan = intCntInvalidPan + 1
                    dtTempView.RowFilter = "PAN = '" & item.Name & "' And IsPANValidationReq = True"
                    For l = 0 To dtTempView.Count - 1
                        dtTempView.Item(l)("PANStatus") = "I"
                        dtTempView.Item(l)("DeducteeStatus") = "Z" '4th Character Updated to 'Z' For Invalid PAN
                    Next
            ...
            Next
                    ...
                    'Commented on 8.002 - Id '0129703' 12/07/19  Start
                    '                For i = 0 To intTempPANCount - 1
                    ...
                    '                Next
                    'Commented on 8.002 - Id '0129703' 12/07/19  End
...
SD:
                    ...
                    Try
                        ...
                        Dim Res1 As JObject = JObject.Parse("{'AEIPL234d': '1', 'BHWPR4467C' : '0', 'BHWPH4467C' : '3'  }")
                        Dim results1 As List(Of JToken) = Res1.Children().ToList
                        For Each item As JProperty In results1
                            item.CreateReader()
                            strVal = ""
                            Try
                                isValid = False
                            ...
                            Catch ex2 As Exception
                                ...
                                intIgnoredPANList = intIgnoredPANList + 1
                                Application.DoEvents()
                        strIngnoredPANList.Append(item.Value.ToString() & vbCrLf)
                        dtTempviewSalary.RowFilter = "PAN = '" & item.Value.ToString() & "' And Filler4='N'"
                        ...
                        If strValidPANList.ToString.Contains(item.Value.ToString()) Then
                        Else
                            strValidPANList.Append("," & item.Value.ToString())
                        End If

                        dtTempviewSalary.RowFilter = "PAN = '" & item.Value.ToString() & "'"
                                'Ver 7.03-REQ816 start
                        For J = 0 To dsMain.Tables("SalaryDetails").Rows.Count - 1
                            If dsMain.Tables("SalaryDetails").Rows(J)("PAN").ToString = item.Value Then
                        ...
                        'Commented on 15/07/19 ver 8.002 Start
                        'For i = 0 To intTempPANCount - 1
                        '    strVal = ""
                        '    If i = 0 Then
                        '        strPanStatus = "Verifying Internet connection..."
                        ...
                        'Next
                        'Commented on 15/07/19 ver 8.002 End
...
EOP:
    ...
    Public Function PanValidation(strPAN As String) As Integer
        ...
        Try
            sReuslt = cp.ProcessIEDriver(strPAN, "Test", "13/07/2019", sError)
        Catch ex As Exception
            MessageBox.Show(sError + " " + ex.Message)
        End Try
        ...
        'blnPANDelegate = True
        ''If (PanValidate.ToString().Contains("This PAN has already been registered.") = True) Then
        ''    Return 1
        ''Else
        ''    Return 0
        ''End If
    ...
    Public Sub WebPageLoad()
        ...
        'pIE1.Navigate(New System.Uri("https://www1.incometaxindiaefiling.gov.in/e-FilingGS/Registration/RegistrationHome.html"))
        'pIE1.Navigate(New System.Uri("https://www.incometaxindiaefiling.gov.in"))
        ...
        'Ver 8.002 End
            ...
            'pIE1.Navigate(New System.Uri("https://authorcopies.peterlang.com/index-stripe.html?ISBN=978-3-03911-420-7&pid=5cebd759b3264d903493ab3d"))
            'pIE1.GoHome()
            'pIEdocHtml = pIE1.Document.DomDocument
            'Dim links = pIEdocHtml.getElementsByTagName("input")
            'For Each link As HtmlElement In links
            '    If link.GetAttribute("class") = "sec_btn" Then
    ...
    'Ver 6.00-REQ660 end
    'Ver 7.03-REQ816 start
    Public Function TaxSlabCalculation(ByVal TotIncome As Double, ByVal isPenalApplied As Boolean, ByVal Category As String) As Dictionary(Of String, Double)
    ...
    End Function
    'Ver 7.03-REQ816 end 
    'Ver 8.002-129703 Start
    Public Function PostDatatoAPI(ByVal endpoint As String, ByVal query As String) As JObject
        'wait for 2 seconds per every web it.
        Dim request As HttpWebRequest = CType(WebRequest.Create(endpoint), HttpWebRequest)
        request.Headers.Add("568d3a5216f75b6HHf9294d9f3", "5690b9fb9f3f0fd74KK63dbfec")
        request.Method = "POST"
        request.ContentType = "application/x-www-form-urlencoded"
        Dim bytes() As Byte = Encoding.UTF8.GetBytes(query)
        ...
        stream.Dispose()
        reader.Dispose()
        Dim webResponse As String = result.ToString
        Return webResponse
    End Function
