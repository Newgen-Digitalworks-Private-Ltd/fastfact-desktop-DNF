...
Public Module CommonModule
    ...
    Public Const ConstStrFYyr As String = "2223"
    ...
    Public strStandaloneDNFPcode As String = "E056"  '' Only for 2223
    'Ver 8.00-FastFacts-652->FastFacts-664 end
    'Ver 18.00-7.00 end
    'Ver 4.041 start
    ...
    Public Const strProductName As String = "eTdsDNF2223"
    'Ver 8.00-FastFacts-652->FastFacts-664 end
    'Ver 18.00-7.00 end
            ...
            Do Until strRd.EndOfStream = True
                ...
                ElseIf mCode = "BH" Then
                    ...
                    If values(4) <> "26Q" And values(4) <> "24Q" And values(4) <> "27Q" And values(4) <> "27EQ" Then
                        ...
                        blnFileClose = True
                        GoTo contn
                    ElseIf values(16) < "201011" Or values(16) > "202223" Then 'Fin Year
...
contn:
        ...
        End Try
    End Sub
    Public Sub NewGetData()
        Dim fs As FileStream
        Dim strRd As StreamReader
        Dim mCode As String = ""
        Dim strLineArray As List(Of String()) = New List(Of String())
        Dim bigdt As Boolean = False
        Dim a As Long = 0
        Try
            If lline > 0 Then GoTo bigdt
            Using stReader As New StreamReader(strFilePath)
                a = 0
                While Not stReader.EndOfStream
                    stReader.ReadLine()
                    a = a + 1
                    If (a > 800000) Then
                        bigdt = True : GoTo bigdt
                    Else
                        bigdt = False
                    End If
                End While
            ...
            If bigdt Then
                ...
                Using stReader As New StreamReader(strFilePath)
                    ...
                    While Not stReader.EndOfStream
                        a = a + 1
                        If a > lline Then
                            strLineArray.Add(stReader.ReadLine().Split("^"))
                            If strLineArray(strLineArray.Count - 1)(1) = "CD" Then
                                k = k + 1
                                If k > 1 Then GoTo setval
                            End If
                        Else
                            If a = 1 Or a = 2 Then
                                strLineArray.Add(stReader.ReadLine().Split("^"))
                    ...
                    lline = 0
setval:
                    For Each values() As String In strLineArray
                        If values.Length > 1 Then
                            mCode = values(1)
                        Else
                            Exit For
                        ...
                        End If
                    Next
                    For i As Integer = 0 To strLineArray.Count - 2
                        strLineArray.RemoveAt(0)
                    Next
                    lline = strLineArray(0)(0) - 1
                End Using
            Else
                fs = New FileStream(strFilePath, FileMode.Open, FileAccess.Read)
                strRd = New StreamReader(fs)
...
contn:
            ...
            For i = 0 To intRateMasterCount - 1
                ...
                For j = 0 To dtTempView.Count - 1
                    ...
                    If dtTempView.Item(j)("DelayedDedMonth") > 0 Then
                        ...
                        dtTempView.Item(j)("DelayedDedAmt") = Math.Round((Val(dtTempView.Item(j)("Deposit")) - (Val(dtTempView.Item(j)("Deposit")) Mod 100)) * dcmDelayeDeductionIntrst * (Val(dtTempView.Item(j)("DelayedDedMonth")) / 12))
                        'Ver 2.01-REQ233 End
                    End If
                    ...
                    If dtTempView.Item(j)("DelayedDepMonth") > 0 Then
                        If strFinYear <> "201011" Then
                            ...
                            dtTempView.Item(j)("DelayedDepAmt") = Math.Round((Val(dtTempView.Item(j)("Deposit")) - (Val(dtTempView.Item(j)("Deposit")) Mod 100)) * dcmDelayeDepositIntrst * (Val(dtTempView.Item(j)("DelayedDepMonth")) / 12))
                            'Ver 2.01-REQ233 End
                            ...
                            If (StrMonth = "03" Or StrMonth = "04" Or StrMonth = "05" Or StrMonth = "06") And dtTempView.Item(j)("DatePayment").ToString().Substring(4, 4) = "2020" Then
                                ...
                                dtTempView.Item(j)("DelayedDepAmt") = Math.Round((Val(dtTempView.Item(j)("Deposit")) - (Val(dtTempView.Item(j)("Deposit")) Mod 100)) * dcmDelayeDepositInt * (Val(dtTempView.Item(j)("DelayedDepMonth")) / 12))
                            End If
    ...
    Public Sub DownLoad(ByVal strFYyr As String, ByVal strURL As String, ByVal strSaveFilePath As String, ByRef pb As ProgressBar)
        ...
        If Application.StartupPath = "C:\eTdsWizard\eTdsDNF" Then
            If Directory.Exists("C:\eTdsWizard\eTdsDNF\Upgrade") = False Then
                Directory.CreateDirectory("C:\eTdsWizard\eTdsDNF\Upgrade")
            End If
        Else
